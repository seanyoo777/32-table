...
> 작성: 2026-06-29 (v4.12 세션) · 대상: 탁구대회 관리 시스템 (32-table)
...
## 1. 현재 상태 (v4.12)
- **HEAD**: 558391f — 전부 push·자동배포(https://32-table.pages.dev), 작업트리 클린.
- **이번 세션 완료 2기능**:
  1. 점수 입력 종목 완료 후 자동 다음 종목 이동
  2. 홈 미확인 점수 기록 뱃지
...
### 다음 후보
- **체크인 탭 선수 검색 하이라이트**: 검색어와 일치하는 이름 글자 강조 표시
- **대회 목록 종목 수·참가자 수 표시**: 카드에 종목 N개·참가자 N명 부가 정보
- **라이브보드 대기경기 코트 배정 표시**: 일정표 슬롯과 연동해 대기 경기에 예정 코트 표시
