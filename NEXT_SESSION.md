...
> 작성: 2026-06-29 (v4.11 세션) · 대상: 탁구대회 관리 시스템 (32-table)
...
## 1. 현재 상태 (v4.11)
- **HEAD**: 177c8e5 — 전부 push·자동배포(https://32-table.pages.dev), 작업트리 클린.
- **이번 세션 완료 2기능**:
  1. 대기경기 행 인라인 코트 번호 선택 (경기별 독립 코트 호출)
  2. 홈 화면 오늘 일정 요약 바 (슬롯 수·첫 경기 시각)
...
### 다음 후보
- **점수 입력 페이지 경기 완료 후 자동 다음 경기 이동**: 현재 다음 미완료 경기로 이동하나 종목 변경 시 수동
- **대시보드 완료 경기 최근 N건 목록**: 현재 완료 수 숫자만, 하단에 최근 결과 목록 카드 추가
- **통계 페이지 이벤트별 메달 집계**: 현재 대회 단위, 종목(event)별 메달 분리 표시
