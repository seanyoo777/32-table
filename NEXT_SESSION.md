...
### 2-38. 체크인 전원 체크인 버튼 (v4.32)
체크인 탭 상단에 "전원 체크인" 버튼 → 등록 선수 전체 일괄 체크인. 대회 직전 빠른 확인용.

### 2-39. 대시보드 최근 호출 이력 (v4.33)
Col 3 하단에 최근 확인된 경기 호출 5건 목록 표시 (acknowledged=true 기준, 시간순 내림차순).

### 2-40. 점수기록 수정 기능 (v4.34)
직접입력 기록 행에 "수정" 버튼 추가 → 승/패자·세트스코어 인라인 편집 후 저장.
