...
> 작성: 2026-06-29 (v4.22 세션) · 대상: 탁구대회 관리 시스템 (32-table)
...
## 1. 현재 상태 (v4.22)
- **HEAD**: 5a3c36a — 전부 push·자동배포(https://32-table.pages.dev), 작업트리 클린.
- **이번 세션 완료 3기능**:
  1. 홈 대회 카드 클릭 → 상세 바로가기 — 928ba91
  2. 체크인 취소 버튼 — 6b90307
  3. 점수기록 세트 상세 펼치기/접기 — 5a3c36a
...
### 다음 후보
- **대시보드 대기 경기 코트 지정 배지**: 일정표에서 배정된 코트 번호를 대기 경기 행에 표시
- **홈 체크인 현황 미니 바**: 홈 상단에 체크인 진행률 요약 표시 (현재 경기 현황 바 옆)
- **점수기록 탭 대회별 통계**: 기록 목록 아래 대회별 세트 평균·최다 득점 등 간단 통계
