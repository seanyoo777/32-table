...
### 2-35. 대시보드 대기경기 대회별 필터 드롭다운 (v4.29)
대기중 경기 컬럼 상단에 대회 드롭다운 추가 → 선택한 대회 경기만 표시. 200+ 경기 적체 시 운영 편의.

### 2-36. 라이브보드 공지사항 배너 (v4.30)
라이브보드(/liveboard)에 운영자가 한 줄 공지문 입력·표시. Zustand persist에 `announcement: string` 필드 추가.

### 2-37. 홈 다음 대기경기 미리보기 (v4.31)
홈 화면 하단에 "다음 호출 예정" 카드 — pendingMatches 상위 3개 경기명·선수 표시.
