...
> 작성: 2026-06-29 (v4.13 세션) · 대상: 탁구대회 관리 시스템 (32-table)
...
## 1. 현재 상태 (v4.12)
- **HEAD**: 9b4c38c — 전부 push·자동배포(https://32-table.pages.dev), 작업트리 클린.
- **이번 세션 완료 1기능**:
  1. 체크인 선수 검색 하이라이트 (이름·학교 매칭 글자 노란 강조)
...
### 다음 후보
- **대회 목록 종목 수·참가자 수 표시**: 카드에 종목 N개·참가자 N명 부가 정보 (Tournament.tsx:186 이미 구현 여부 재확인 필요)
- **라이브보드 대기경기 코트 배정 표시**: `ScheduleSlot`에 `matchId` 필드 없음(matchNo만 있음) → 먼저 types/index.ts ScheduleSlot에 `matchId?: string` 추가 + scheduleUtils 슬롯 생성 시 matchId 주입 필요. 그 후 Liveboard에서 연동.
- **체크인 현장등록**: 미체크인 선수 당일 현장등록 워크플로 (이름·소속 입력 → 즉시 체크인)
