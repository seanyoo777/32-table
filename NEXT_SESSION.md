...
> 작성: 2026-06-29 (v4.10 세션) · 대상: 탁구대회 관리 시스템 (32-table)
...
## 1. 현재 상태 (v4.10)
- **HEAD**: ea7c4ee — 전부 push·자동배포(https://32-table.pages.dev), 작업트리 클린.
- **이번 세션 완료 2기능**:
  1. 대시보드 코트 현황 LIVE/호출 카운터 (없으면 숨김)
  2. 점수 직접입력 진행중 대회 자동 선택
...
### 다음 후보
- **라이브보드 종료 경기 누적 표시**: 완료된 경기 결과를 하단에 접이식 목록으로 표시
- **대시보드 대기경기 빠른 호출**: 대기 경기 행에 코트 선택 드롭다운 + 호출 버튼 인라인
- **홈 화면 오늘 일정 요약**: 오늘 등록된 schedule 슬롯 수·시작 시간 표시
