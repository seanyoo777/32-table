  ...
  return (
    <div className="page-shell p-4 gap-3">
      ...
      {todaySlotCount > 0 && (() => {
        ...
        const slotMins = (hhmm: string) => { const [h, m] = hhmm.split(':').map(Number); return h * 60 + m }
        const allSlots = todaySchedules
          ...
          <div className="flex-shrink-0 space-y-1.5">
            ...
            {recentDone.map(sl => (
              <button key={sl.id} onClick={() => navigate('/schedule')}
                ...
                <span className="text-xs font-bold text-gray-400 w-10 flex-shrink-0">{sl.startTime}</span>
                <span className="text-[10px] text-gray-400 w-8 flex-shrink-0">코트{sl.courtNo}</span>
                <span className="flex-1 text-xs text-gray-500 truncate">{sl.participant1} vs {sl.participant2}</span>
                ...
                return (
                  <div key={t.id}
                    ...
                    {(() => {
                      ...
                      const morning = ci.filter(p => p.createdAt && new Date(p.createdAt).getHours() < 12).length
                      const afternoon = ci.filter(p => p.createdAt && (() => { const h = new Date(p.createdAt).getHours(); return h >= 12 && h < 18 })()).length
                      const evening = ci.filter(p => p.createdAt && new Date(p.createdAt).getHours() >= 18).length
                      const items = [
                      ...
                      return (
                        <div className="mt-1">
                          <div className="flex h-1.5 rounded-full overflow-hidden gap-px">
                            {items.map(i => (
                              <div key={i.label} className={`${i.color}`} style={{ width: `${Math.round(i.count / total * 100)}%` }} title={`${i.label} ${i.count}명`} />
                            ))}
                          </div>
