...
> **버전**: v5.77 | **스택**: Vite + React 18 + TypeScript + Tailwind CSS + Zustand  
...
## 12-184. v5.77 — 홈 최근 완료 경기 칩

자동 루프. Home.tsx.

### 변경 (Home.tsx)
- `scoreRecords.reverse().slice(0,3)` → 승자+점수 green 칩 행.
- 체크인 현황 바 위에 삽입.

---

## 12-183. v5.76 — 랭킹 학교별 그룹 보기

자동 루프. Rankings.tsx.

### 변경 (Rankings.tsx)
- `groupBySchool` state + 체크박스.
- school 헤더 tr + 소속 선수 tr 교대 렌더링.

---

## 12-182. v5.75 — 대시보드 LIVE 스코어 입력 버튼

자동 루프. Dashboard.tsx.

### 변경 (Dashboard.tsx)
- LIVE 카드 하단 `⚡ 스코어 입력 →` 버튼.
- stopPropagation + navigate('/score').
...
## 12-163. v5.56 — 대회 경기 목록 선수명 랭킹 이동

자동 루프. Tournament.tsx MatchList 선수명 클릭.

### 변경 (Tournament.tsx)
- `useNavigate` import + MatchList 컴포넌트에 훅 추가.
- P1/P2 이름 `onClick` + `stopPropagation`. `/rankings?search=NAME` navigate.

---

## 12-162. v5.55 — 경기일정 슬롯 검색 필터

자동 루프. Schedule.tsx 헤더 검색 input.

### 변경 (Schedule.tsx)
- `slotSearch` state 추가.
- `filteredSlots` 필터에 검색어 조건 추가.
- 헤더 input + ✕ 초기화 UI.

---

## 12-161. v5.54 — 통계 선수 포인트 분포 히스토그램

자동 루프. Stats.tsx 포인트 5구간 막대.

### 변경 (Stats.tsx)
- `players.filter(p => p.points > 0)` 기준 maxPt/bucketSize 계산.
- 5개 구간 가로 막대 + 구간 레이블 + 명 수.
...
자동 루프 세션. Dashboard.tsx 통계 행 아래 시간별 미니 막대 추가.

### 변경 (Dashboard.tsx)
- 오늘 scoreRecords를 0-23시 배열로 집계.
- 24개 막대: 현재 시각대=보라, 기록 있음=연보라, 없음=회색.
- 막대 높이: 최대 20px 기준 비례. 최소 2px.
- 오늘 기록 0건이면 행 숨김.

---

## 12-105. v4.98 — 랭킹 미체크인 행 주황 강조

자동 루프 세션. Rankings.tsx 미체크인 선수 행 배경 강조.

### 변경 (Rankings.tsx)
- 체크인한 선수가 1명 이상 있을 때만 적용.
- 미체크인 행: `bg-orange-50`.
- 상위 3명 노란 행과 OR 조건으로 처리.

---

## 12-104. v4.97 — 홈 대회 카드 종목 태그 완료율

자동 루프 세션. Home.tsx 대회 목록 이벤트 칩에 완료% 인라인 추가.

### 변경 (Home.tsx)
- 이벤트 칩 `{ev.label}` 옆에 `{evPct}%` 소형 폰트 추가.
- 경기 없는 이벤트는 % 미표시.
- 100% 완료 이벤트: `bg-green-100 text-green-700` 칩 색상.
